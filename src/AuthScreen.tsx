...
export default function AuthScreen({ mode, setMode, notice, onAuthenticated, onCompanySelected }: Props) {
  ...
  return <div className="auth-shell">
    ...
    <div className="auth-card">
      ...
      <p className="auth-subtitle">{mode === 'register' ? 'Создайте рабочее место хозяйства. В существующее хозяйство агроном входит по приглашению владельца.' : 'Войдите, чтобы работать с полями вашего хозяйства. Если новая PostgreSQL пуста, зарегистрируйте аккаунт заново: старые данные не переносятся автоматически.'}</p>
      ...
      <form onSubmit={(event) => void submit(event)}>
        ...
        {mode === 'register' && <>
          <label className="form-label">Рабочее место<select className="form-input" value={companyMode} onChange={(event) => setCompanyMode(event.target.value as 'new' | 'existing')}><option value="new">Создать новое хозяйство</option><option value="existing">Войти в существующее по приглашению</option></select></label>
          {companyMode === 'new' ? <>
            <label className="form-label">Название хозяйства<input className="form-input" value={companyName} maxLength={120} onChange={(event) => setCompanyName(event.target.value)} required /></label>
            <label className="form-label">БИН (необязательно, если он есть)<input className="form-input" inputMode="numeric" value={companyBin} maxLength={12} onChange={(event) => setCompanyBin(event.target.value.replace(/\D/g, ''))} placeholder="Можно оставить пустым" /><small>Если укажете БИН, проверьте его контрольную цифру.</small></label>
