...
export function buildFieldReportHtml(company: string, field: ReportField, weather: ReportWeather | null, tasks: ReportTask[], generatedAt = new Date(), tasksUnavailable: string | null = null, indices: IndexResponse[] = []) {
  ...
  return `<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SmartAgro — ${escape(field.name)}</title><style>
    ...
    <section><h2>Урожай и индексы</h2><p>Собрано: ${escape(field.harvestTotalT)} т. Фактическая урожайность: ${escape(measure(actualYield, 'т/га'))}.</p><p>Прогноз урожайности, доверительный интервал и оценка климатических рисков: нет данных — расчётные модели не подключены.</p><div style="overflow-x:auto"><table><thead><tr><th>Индекс</th><th>Последнее среднее по полю</th><th>Дата интервала</th><th>Источник</th><th>Происхождение</th><th>Качество</th></tr></thead><tbody>${indexRows}</tbody></table></div>${indexChart}<p class="source">CDSE: статистика Sentinel-2 L2A по контуру поля с маской SCL; дата обозначает дневной интервал расчёта, а не время конкретного снимка. CSV: данные загружены пользователем, происхождение не проверено. Растровой карты внутриполевых зон нет. Источник сбора: ввод агронома от ${escape(date(field.updatedAt))}. Фактическая урожайность = собранный урожай / посеянная площадь.</p></section>
